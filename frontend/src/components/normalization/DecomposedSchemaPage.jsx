...
import { Database, Table as TableIcon, X } from "lucide-react";

export default function DecomposedSchemaPage({ result, flatFileData }) {
        ...
        {result.decomposedRelations.map((rel, i) => {
            ...
            <div 
              ...
              <div style={{ flex: 1, overflow: "auto", border: "1px solid #2a292f", borderRadius: "8px", background: "#0e0e13" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px", fontFamily: "Fira Code, monospace", color: "#e4e1e9" }}>
                  <thead style={{ position: "sticky", top: 0, background: "#1f1f25", zIndex: 1 }}>
                    <tr>
                      {rel.attributes.map(attr => (
                        <th key={attr} style={{ padding: "8px 12px", textAlign: "left", borderBottom: "1px solid #3a494a", color: "#63f7ff", fontWeight: 700 }}>
                          {attr}
                        </th>
                      ))}
                    </tr>
